...
class CollectorService(os_service.Service):
    ...
    def start_udp(self):
        # LOG.debug("From byhan : running start_udp")
        ...
        while self.udp_run:
            ...
            # enough for anybody.
            # LOG.debug("From byhan : running udp_run")
            data, source = udp.recvfrom(64 * units.Ki)
            try:
                sample = msgpack.loads(data, encoding='utf-8')
                # LOG.debug("From byhan : printing sample : %s", sample)
            except Exception:
                LOG.warning(_("UDP: Cannot decode data sent by %s"), source)
            else:
                try:
                    LOG.debug("UDP: Storing %s", sample)
                    # LOG.debug("From byhan : printing sample in else section : %s", sample)
...
class CollectorEndpoint(object):
    ...
    def sample(self, messages):
        ...
        # LOG.debug("from byhan: printing samples = %s", samples)
        try:
